...
function InitInstantPage() {
    ...
    if (!supportsPrefetch) {
        console.log('instant.page: browser does not support prefetch')
        return
    }
    ...
    if (!isBrowserSupported) {
        console.log('instant.page: browser does not support prefetch or modulepreload')
        return
    }
    ...
    if ('instantIntensity' in document.body.dataset) {
        ...
        if (intensityParameter == 'viewport') {
            ...
            const isNavigatorConnectionSaveDataEnabled = nav.connection && nav.connection.saveData
            const isNavigatorConnectionLike2g = nav.connection && nav.connection.effectiveType && nav.connection.effectiveType.includes('2g')
    ...
    if (preloadWhenVisible) {
        let requestIdleCallbackOrFallback = window.requestIdleCallback as any;
            ...
            const intersectionObserver = new IntersectionObserver((entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        const anchorElement = entry.target as any;
    ...
    function preload(url, fetchPriority : 'auto' | 'high' | 'low' = 'auto') {
        console.log('instant.page preload', url)
        if (_preloadedList.has(url)) {
            console.log('instant.page preload: already preloaded', url)
            return
        }
    ...
    function preloadUsingLinkElement(url, fetchPriority : 'auto' | 'high' | 'low' = 'auto') {
